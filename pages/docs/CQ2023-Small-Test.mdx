...
基础：A11支持UDC gota升级 A版本到C版本的差分包： 正常用target包做即可，按理来说是OK的，本项目是客户配置的gota服务器，所以未进行验证，后续可以尝试验证下 
B版本到C版本的差分包： 因为B版本是打包的udc的pac，所以没有target包，复制A版本的target包出来，替换target包中的udc.bin和UDC_config.img为C版本中的（参照FAQ:删除zip中特定文件、zip新增文件）, 然后在使用target包做差分包，验证升级OK 
